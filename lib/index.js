...
const scrollParent_1 = __importDefault(require("./utils/scrollParent"));
const debounce_1 = __importDefault(require("./utils/debounce"));
const throttle_1 = __importDefault(require("./utils/throttle"));
...
const checkVisible = function checkVisible(component) {
    ...
    const parent = (0, scrollParent_1.default)(node);
...
class LazyLoad extends react_1.Component {
    ...
    componentDidMount() {
        ...
        if (needResetFinalLazyLoadHandler) {
            (0, event_1.off)(scrollport, 'scroll', finalLazyLoadHandler, passiveEvent);
            (0, event_1.off)(window, 'resize', finalLazyLoadHandler, passiveEvent);
            finalLazyLoadHandler = null;
        }
        if (!finalLazyLoadHandler) {
            if (this.props.debounce !== undefined) {
                finalLazyLoadHandler = (0, debounce_1.default)(lazyLoadHandler, typeof this.props.debounce === 'number' ? this.props.debounce : 300);
                delayType = 'debounce';
            }
            else if (this.props.throttle !== undefined) {
                finalLazyLoadHandler = (0, throttle_1.default)(lazyLoadHandler, typeof this.props.throttle === 'number' ? this.props.throttle : 300);
                delayType = 'throttle';
            }
            else {
        ...
        if (this.props.overflow) {
            const parent = (0, scrollParent_1.default)(this.ref);
            ...
            if (scroll) {
                (0, event_1.on)(scrollport, 'scroll', finalLazyLoadHandler, passiveEvent);
            }
            if (resize) {
                (0, event_1.on)(window, 'resize', finalLazyLoadHandler, passiveEvent);
            }
        }
        listeners.push(this);
        checkVisible(this);
    ...
    componentWillUnmount() {
        ...
        if (listeners.length === 0 && typeof window !== 'undefined') {
            (0, event_1.off)(window, 'resize', finalLazyLoadHandler, passiveEvent);
            (0, event_1.off)(window, 'scroll', finalLazyLoadHandler, passiveEvent);
        }
    }
    setRef(element) {
        if (element) {
    ...
    render() {
        ...
        return (react_1.default.createElement("div", { className: `${classNamePrefix}-wrapper ${className}`, ref: this.setRef, style: style }, this.visible ? (children) : placeholder ? (placeholder) : (react_1.default.createElement("div", { style: { height: height }, className: `${classNamePrefix}-placeholder` }))));
    }
}
LazyLoad.propTypes = {
...
const decorator = (options = {}) => function lazyload(WrappedComponent) {
    return class LazyLoadDecorated extends react_1.Component {
        constructor() {
            super();
            this.displayName = `LazyLoad${getDisplayName(WrappedComponent)}`;
        }
        render() {
            return (react_1.default.createElement(LazyLoad, Object.assign({}, options),
